...
export class GameStateService implements OnDestroy {
  ...
  private wireRoomEvents(): void {
    ...
    this.subs.add(this.room.roundStart$.subscribe(ev => {
      ...
      // Timer driven by answerWindowEndsAt if available, else fallback to 30s
      this.startTimerFromDeadline(this.room.answerWindowEndsAt$.getValue(), 30);
    }));

    // buzz_open → start answer-window timer precisely from server deadline
    this.subs.add(this.room.buzzOpenEvent$.subscribe(ev => {
      this.startTimerFromDeadline(ev.answerWindowEndsAt, ev.answerWindowMs / 1000);
    }));

    // buzz_accepted → reaction + banner
    this.subs.add(this.room.buzzAccepted$.subscribe(ev => {
      this.addReaction(ev.sessionId, ev.nickname, '⚡');
    }));

    // answer_result → score, streak, effects
    this.subs.add(this.room.answerResult$.subscribe(ev => {
      this.stopTimer();
      this.totalAttempts++;
      ...
      if (ev.correct) {
        ...
        this.addHistory(ev.answer, ev.pointsAwarded, (ev.timeTakenMs / 1000).toFixed(1), true);
      } else {
        this.myStreak$.next(0);
        this.showToast('WRONG ANSWER!', 'error');
        this.wrongFlash$.next();
        this.triggerShake();
        this.addHistory(ev.answer, 0, (ev.timeTakenMs / 1000).toFixed(1), false);
      }
    ...
    this.subs.add(this.room.roundEnd$.subscribe(ev => {
      this.stopTimer();
    ...
    // match_end
    this.subs.add(this.room.matchEnd$.subscribe(ev => {
      this.stopTimer();
    ...
    // connection error
    this.subs.add(this.room.error$.subscribe(err => {
  ...
  buildPuzzleFromWord(scrambled: string): void {
    ...
    const grid: LetterTile[] = letters.map((l, i) => ({
      id: uid(), letter: l, used: false, source: 'grid', gridIndex: i,
    }));

    const shuffled = [...letters].sort(() => Math.random() - 0.5);
    const bank: LetterTile[] = shuffled.map(l => ({
      id: uid(), letter: l, used: false, source: 'bank',
    }));

    const slots: AnswerSlot[] = letters.map((_, i) => ({
      index: i, letter: '', tileId: null, active: i === 0, filled: false,
    }));
  ...
  addLetterFromGrid(tile: LetterTile): void {
    ...
    slots[nextEmpty] = { ...slots[nextEmpty], letter: tile.letter, tileId: tile.id, filled: true, active: false };
  ...
  removeSlotAt(index: number): void {
    ...
    slots[slots.length - 1] = { index: slots.length - 1, letter: '', tileId: null, active: false, filled: false };
  ...
  clearSlots(): void {
    this.gridTiles$.next(this.gridTiles$.getValue().map(t => ({ ...t, used: false })));
    this.bankTiles$.next(this.bankTiles$.getValue().map(t => ({ ...t, used: false })));
    this.answerSlots$.next(this.answerSlots$.getValue().map((s, i) => ({
      ...s, letter: '', tileId: null, filled: false, active: i === 0,
    })));
  }

  shuffleBank(): void {
    const bank = [...this.bankTiles$.getValue()];
    const available = bank.filter(b => !b.used).sort(() => Math.random() - 0.5);
    const used      = bank.filter(b =>  b.used);
    this.bankTiles$.next([...available, ...used]);
  }

  getAnswerWord(): string {
    return this.answerSlots$.getValue().map(s => s.letter).join('');
  }
  ...
  startTimer(seconds: number): void {
    ...
    this.timerInterval = setInterval(() => {
      const v = parseFloat(Math.max(0, this.timerValue$.getValue() - 0.1).toFixed(1));
  ...
  addReaction(sessionId: string, nickname: string, emoji: string): void {
    ...
    setTimeout(() => {
      this.reactions$.next(this.reactions$.getValue().filter(x => x.id !== r.id));
    }, 2500);
  }

  triggerReaction(emoji: string): void {
    const nickname = this.room.getStateSnapshot()
      ?.players.find(p => p.customerId === this.room.myCustomerId$.getValue())
      ?.nickname ?? 'You';
  ...
  addFloatingScore(value: number): void {
    ...
    setTimeout(() => {
      this.floatingScores$.next(this.floatingScores$.getValue().filter(s => s.id !== fs.id));
    }, 1600);
  }
  ...
  addHistory(word: string, pts: number, time: string, correct: boolean): void {
    const h = [{ word, pts, time, correct }, ...this.history$.getValue()].slice(0, 8);
    this.history$.next(h);
  }
  ...
  private recalcAccuracy(): void {
    ...
    this.myAccuracy$.next(Math.round((this.correctCount / this.totalAttempts) * 100));
  }
