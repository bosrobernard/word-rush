...
export class GameRoomService implements OnDestroy {
  ...
  // ── Actions ───────────────────────────────────────────────────────────────
  buzz(roundNo: number):                 void { this.client?.sendBuzz(roundNo); }
  answer(roundNo: number, word: string): void { this.client?.sendAnswer(roundNo, word); }
  forfeit():                             void { this.client?.sendForfeit(); }

  // ── Helpers ───────────────────────────────────────────────────────────────
  getScoreboard():       ScoreboardItem[]        { return this.client?.getScoreboard()       ?? []; }
  getConnectedPlayers(): PlayerSnapshot[]        { return this.client?.getConnectedPlayers() ?? []; }
  getStateSnapshot():    RoomStateSnapshot|null  { return this.client?.getStateSnapshot()    ?? null; }
  isMe(sessionId: string): boolean               { return sessionId === this.mySessionId$.getValue(); }
