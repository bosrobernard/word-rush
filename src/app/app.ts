...
import { FindMatchOverlay } from './shared/find-match-overlay/find-match-overlay';
...
@Component({
  ...
  template: `
    ...
    <!-- LOBBY (auth only) — shown until logged in -->
    <app-lobby *ngIf="!showGameUI" (joined)="onJoined()"></app-lobby>

    <!-- GAME screen — shown once logged in (preview or live) -->
    <div id="app" *ngIf="showGameUI">
    ...
    <!-- FIND MATCH overlay — shown when logged in but not yet connected -->
    <app-find-match-overlay
      *ngIf="showGameUI && !connected"
      (joined)="onJoined()"
      (logout)="onLogout()"
    >
    </app-find-match-overlay>
  ...
  styles: [
    `
      ...
      .conn-error {
        ...
        background: rgba(224, 51, 68, 0.15);
        border: 1px solid rgba(224, 51, 68, 0.4);
        border-radius: 12px;
        padding: 10px 20px;
...
export class App implements OnInit, AfterViewInit, OnDestroy {
  ...
  get showGameUI(): boolean {
    return this.connected || !!localStorage.getItem('lb_access_token');
  }
  ...
  ngOnInit(): void {
    this.gameState.seedPreviewState();

    this.subs.add(
      this.room.connected$.subscribe((v) => {
        this.connected = v;
    ...
    this.subs.add(
      this.room.connectionError$.subscribe((err) => {
